...
If you have cameras in the corners, they should likely be upright for a wide field of view and greatest coverage.
Any additional cameras mounted at the edges might benefit from a sideways mount for a tall field of view if you want tracking close to the wall.
...
To increase usefulness during setup, you may use the contrast and brightness settings in the "Visualisation" panel, Section "Image Adjustment" to make the image more visible.
You can maximise the camera view by pressing the detach button in the top left of the camera view, then pressing it again to maximise the area.

## Custom Lenses
The default lenses right now have a high field of view, to improve ease of setup in VR-related use cases, but they exhibit significant distortions that need to be compensated for, ultimately being a compromise between usability and absolute tracking quality. <br>
Professional tracking systems often use narrower field of views with less distortions, giving them more range and accuracy in larger tracking spaces, but also requiring more cameras and careful planning of the setup. <br>
The current default lenses have an field of view of (89°h, 51°v, 108°d) including distortions, but due to about 128 horizontal pixels being unused by processing, the effective tracking field of view is (78°h, 51°v, 99°d). <br>
The lenses can be relatively easily exchanged for custom setups, the default ones are manually secured with a fixing ring and light glue which can be easily removed. <br>
...
#### Lens Compatibility
The camera has a filter switcher, and it does limit the kinds of lenses you can use.
Some complex lenses that compensate for distortion may have a focal length smaller than the height of the filter switcher, and thus can not be focused without removing the filter switcher.
If you must use such a lens, you may need to order one with an IR-pass filter around 850nm attached or carefully attach one yourself, and replace the filter switcher assembly with another suitable M12 lens mount. <br>
Additionally, the total lens height once focused needs to roughly line up with the IR LED board height, lest the IR light leaks into the lens and washes out the image, the IR LED board occludes part of the image, or the lens occludes portions of the usable LED light.
Generally, lenses with much higher or lower field of view than the default may not line up.
...
Lenses with a wider field of view may also have a wider lens head that may exceed the size of the throat of the IR LED Board (21mm in the current design).

#### Focusing the Lens
For practical reasons (the IR LED board hindering access to the lens) it is recommended to focus the lens without relying on the retroreflection from the IR LEDs. <br>
This can be achieved by using an IR LED, or a visible light LED with the IR-pass filter disabled.
You may need to occlude a larger LED behind a pinhole to ensure a point-like quality - this will greatly aid in achieving a proper focus. <br>
...
After achieving a rough focus, you may zoom in on the point (Ctrl + Scroll, Ctrl + Drag), and find the exact focus - you should be able to see the [pixel instability of the OV9281 sensor](../operation/issues.md#ov9281-pixel-instability) at maximum focus.
...
Finally, if you are not familiar with a fixing ring, you need to bring the ring to the estimated position first, so that you can tighten the lens against it.
Doing so will disturb your intended focus point, so you may need to iteratively find the right place for the fixing ring so that the lens achieves a good focus when tightened against the ring.