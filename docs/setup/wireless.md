...
The cameras can connect to a local wireless network as an alternative communication channel for high-throughput data like a high-quality video stream. That, in addition to allowing for remote development via SSH, has been its primary purpose. <br>
...
This will only be stored in the software for this session and is not retained as the cameras are expected to store that information. <br>
...
You can also add multiple networks in case you frequently move the cameras around. <br>
These credentials are only sent to cameras with enabled wifi, and stored/updated persistently only when you explicitly apply their config in their action dropdown menu.
If you are connected to a camera only through the wireless server, specifying invalid credentials and hitting "Apply" will disconnect the camera, and you may need to manually cut power to it for it to restart and reconnect with the previously stored wireless network configuration.

## Wireless Config
In the wireless camera table, you can configure and inspect the wireless status of all connected cameras. <br>
<br>
The first few columns are wireless feature toggles (use "Apply Config" action to make them persist): <br>
**Toggle Wifi:** Connect to a configured wireless network. <br>
**Toggle SSH:** Enable OpenSSH access for [remote development work](#remote-development-via-ssh) on the camera. <br>
...
Finally, you may permanently disable Wifi and SSH. These can only be undone by either flashing firmware onto the camera that ignores/undoes these checks, or by taking the camera apart and modifying the SD card, so be careful. <br>

## Server Config
Here you may edit the default host and port the cameras try to connect to when they are configured to connect to the wireless server. <br>
...
Your best bet is to remove AsterTrack from the list entirely, restart the AsterTrack Server Software, and start the wireless server ("Connect") to bring up the popup again. <br>
...
This may be a failure of DHCP on your host system making the router aware of your hostname in the first place, or a DNS failure due to misconfiguration of the router.
...
It is primarily intended for home use where setting up certain cable runs is prohibitive, and turning one or multiple cameras wireless is acceptable.
...
- `filetool.sh -b` to saving your work (changes, build, shell history) to SD card<br>
- configuration is currently stored in `/mnt/mmcblk0p2/config`
...
- to recompile camera kernel drivers (e.g. to modify them):
    ...
    - `./drivers_load_modules.sh`: registers and loads the newly built driver module (at startup)
        - You can not replace a currently loaded driver. Ensure this script refers to the drivers you want loaded, save your work with `filetool.sh -b`, and restart.
