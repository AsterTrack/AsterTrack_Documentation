...
For full details, refer to the schematics once they are finalised and released.
...
If configured, and External Power is not otherwise powered, the USB PD power may also be exposed on the External Power port-

#### UART Communication
Both the SBC (Raspberry Pi) and MCU on the camera are set up to be able to receive and transmit on the UART. <br>
The MCU needs to communicate with the controller during startup, the SBC once it has booted up.
During streaming, the MCU needs to receive timing packets from the UART, the SBC needs to receive control packets from the UART. <br>
These were the original design constraints, though since then, another complicated matters:
The PL011 UART Transceivers used in the Raspberry Pis does NOT use DMA in the driver due to implementation issues, and the FIFO is too small to reliably receive packets at 8MBaud when the CPU is under load.
...
In practice, the UART is MUXed, defaulting to the Pi, allowing the Pi to function while the MCU is reset, flashed, or bricked.
...
Though currently, even this algorithm, called "resegmentation", is applied to all small blobs, so there is a lot of potential to increase speed. <br>
For large blobs (comprised of more than 1000 pixels currently) there is an alternate blob refinement strategy that fits a circle to the image blob.
This can not deal with distortions well, and is not relied upon for most tracking tasks.

#### Packet Hub (Controller)
There is some consideration put into distributing the incoming UART packets from all 8 ports to the USB interface, though with the switch from multiple Full-Speed Interrupt Endpoints to a single High Speed Endpoint, this has gotten a lot less complicated, with less focus on timing the next USB transfer. <br>
...
Large packets (currently of size 256 or higher) are being DMAd right from the UART buffer.
Smaller packets are copied (CPU or DMA) into a shared packet to be sent together.
There is potential to extensively instrument, monitor, and optimise this, in an effort to improve both throughput and latency, but the current solution has in practice no troubles handling the full throughput of the 8 UART ports.

#### Streaming Subsystem (Server)
The server receives the streaming data (referring to the realtime stream of blobs for each frame) in chunks by each camera.
...
Part of this is also the requirement that cameras announce they are processing a frame, to not wait on cameras that had to drop a frame due to potential processing shortfalls.
Currently, the system is configured to prioritise realtime use, with processing being started even when camera are still expected to send data if they exceed certain thresholds, and delayed data being discarded. <br>
In the future, delayed data will be properly recorded so that tracking in post may perform better than in realtime.

#### Camera Calibration (Pipeline)
While the final calibration (including from future continuous calibration) is mainly a result of optimisation alone, the initial reconstruction of camera transforms and linear lens parameters is more complex.
It is also a major part in determining how easy-to-use a system is, and what it may support in terms of large, multi-room tracking volumes (latter can be supported with some further work, do reach out if required). <br>
...
This has been extensively researched and implemented as a Bachelor Thesis, which you may request by reaching out.
The final method being used is described in "A Convenient Multicamera Self-Calibration for Virtual Environments" by Tomáš Svoboda, Daniel Martinec, and Tomáš Pajdla, with prior work from, among others, David Jacobs, Peter Sturm, Bill Triggs, Hugh Christopher Longuet-Higgins and Richard I. Hartley.
...
The initial reconstruction is also currently completely insufficient, with the resulting estimate rarely being more than a random arrangement of markers, though some effort has been put into their algorithm.
That means, current target calibration mostly relies on good data aquisition, and compute-intensive optimisation, in hopes of getting matching estimates that can be merged to assemble the full set of markers that make up the target.
