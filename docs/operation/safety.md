...
That means, we can maximise the strength of the pulse while minimising the exposure duration for optimal isolation from other IR sources. <br>
...
The regulators are not designed to limit it to a safe power level, and the firmware limitations can not be guaranteed to be safe either before further testing with exact, final hardware. <br>
