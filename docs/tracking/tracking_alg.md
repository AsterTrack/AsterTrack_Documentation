...
### Prediction
We use a mathematical model - either the Extended or Unscented Kalman Filter - to estimate and predict the state of each tracker at the time of the current frame.
This gives us an initial position and orientation to start tracking.
...
In that case, we correct the filter (UKF update) using the pose itself as the measurement. <br>
...
## Debugging Tracking
There are several ways to debug the tracking pipeline, and many of them are only really relevant during [replay](../operation/recording.md#replay), when frame processing can be halted and examined in detail.
...
To get the most out of this tooling, the tracking needs to be deterministic. Replay mode already does not skip frames, and instead slows down replay if required, but detection may still be asynchronous and thus differs from run to run. So you may have to turn async search/probe off in the "Parameters/Tracking" panel under section "Target Detection".