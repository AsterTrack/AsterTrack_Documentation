...
## I/O Protocols
I/O protocols are the way to stream tracking data out of the AsterTrack Server Software and towards various clients in real-time.
...
In the AsterTrack Server Software, these integrations can be configured in the "Integrations" panel.
...
#### VMC
[VMC](https://protocol.vmc.info/english.html) is a protocol built upon [OSC](https://opensoundcontrol.stanford.edu) designed for motion capture and similarly uses UDP to send tracker updates.
While mostly designed for sending bone transforms and face tracking information, it also supports 6-DOF trackers and can in theory be extended to convey more specific information. In contrast to VRPN it is a one-way one-on-one communication, the sender does not know if the connection is set up properly, and just blindly sends to the specified client. <br>
It is widely supported by game and modeling software via integrations, and natively by various vtubing software.
VMC (by relying on OSC) is simple to implement and has options in various languages.
...
This will see further expansion to support virtual production usecases, and receive full support as an addon in the future - for now, follow the [setup instructions](https://github.com/AsterTrack/Clients/blob/main/examples).
...
While technically just implementing the [VRPN I/O protocol](#vrpn), it supports AsterTrack specific features like sending IMU samples to the AsterTrack Server Software. <br>
...
The plan is that you can retrofit any monado-supported VR hardware with retroreflective markers, calibrate them, set up the VRPN connection, and the IMUs are automatically calibrated so the tracking spaces align themselves.


## File Formats
For offline, non-realtime post-processing work, a capture and/or its tracking results can be exported to file formats used by relevant third-party software.
